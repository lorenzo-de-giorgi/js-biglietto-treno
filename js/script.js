...
if((!isNaN(km)) && (!isNaN(age))){
    ...
    if(age < 18){
        ...
        document.getElementById('app').innerHTML = `<h2>Il costo del biglietto per ${km} per una persona di ${age} anni è: ${finalPriceM}</h2>`
    } else if(age > 65){
        // biglietto over 65
        ...
        document.getElementById('app').innerHTML = `<h2>Il costo del biglietto per ${km} per una persona di ${age} anni è: ${finalPriceO}</h2>`
    } else {
        //biglietto normale
        console.log(`il costo del biglietto intero senza sconti è: ${costoNormal}`)
        document.getElementById('app').innerHTML = `<h2>Il costo del biglietto per ${km} per una persona di ${age} anni è: ${costoNormal}</h2>`
    }
} else {
    //input sbagliato
